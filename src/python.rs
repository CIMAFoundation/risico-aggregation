...
// Import GenericBBox and Point from the shapefile crate (adjust as needed).
use shapefile::Point;
...
impl PyGeomRecord {
    ...
    pub fn new(
        ...
        // For simplicity, assume the geometry string contains two space-separated numbers for a Point.
        let coords: Vec<f64> = geometry
            .split_whitespace()
            .filter_map(|s| s.parse::<f64>().ok())
            .collect();
        if coords.len() < 2 {
            return Err(PyErr::new::<exceptions::PyValueError, _>(
                "Invalid geometry string; expected at least two numbers",
            ));
        }
        let point = geo_types::Point::new(coords[0], coords[1]);
        let geom = geo_types::Geometry::Point(point);
        // Convert the bbox tuple into a GenericBBox.
        let bbox_converted = GenericBBox {
            min: Point::new(bbox.0, bbox.1),
            max: Point::new(bbox.2, bbox.3),
        };
...
pub fn py_calculate_stats(
    ...
    let rust_stats_functions: Vec<StatsFunctionType> = stats_functions
        ...
        .map(|s| {
            s.parse().map_err(|_| {
                PyErr::new::<exceptions::PyValueError, _>(format!("Invalid stats function: {}", s))
            })
        })
        .collect::<Result<_, _>>()?;
